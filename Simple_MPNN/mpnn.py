...
class Model(nn.Module):
    '''
    Model class representing your MPNN and it inherits from nn.Module.
    '''
    def __init__(self):
        '''
        ...
        self.raw_features = 119  # 119 is number of periodic elements
                                 # An atom is transform to 1 hot vector with length = 119
        self.lift_features = 300
        self.num_classes = 2  # molecule has 2 labels: 1 and 0
        self.batch_size = 10
    ...
    def forward(self, g):
        ...
        Analagous to "call" methods in Tensorflow.
...
class MPLayer(nn.Module):
    ...
    def forward(self, g, node_feats):
        '''
        Responsible for computing the forward pass of your network.
        Analagous to "call" methods from tensorflow.
        ...
        :param 	node_feats: Beginning node features for your graph.
                            should be a torch tensor (float) of shape
    ...
    def message(self, edges):
        ...
        :return	A map from some string (you choose) to all the messages computed for each edge.
                ...
                (e.g destination_node.mailbox["string_you_chose"]) once DGL distributes
                them with the send function).
        '''
...
def train(model, train_data):
    ...
    :param train_data: A 1-D list of molecule objects, representing all the molecules in the training set
                       from get_data
    :return	nothing.
    '''
    ...
    for batch in range(N):
        ...
        if (batch + 1) * model.batch_size > len(train_data):
            end = len(train_data)
        ...
        graphs = dgl.batch(gh)
        logits = model(graphs)
        labels = torch.tensor(np.array(lb), dtype=torch.long)
...
def test(model, test_data):
    ...
    for batch in range(N):
        ...
        if (batch + 1) * model.batch_size > len(test_data):
            end = len(test_data)
...
def main():
    ...
    train_data, test_data = get_data('data/1-balance.sdf')
    model = Model()
    epochs = 100
    for epoch in range(epochs):
        now = datetime.now()
        print(f'Epoch {epoch} at {now}')
        train(model, train_data)
        finish = datetime.now()
        print(f'training end at {finish}: takes = {finish-now}')
